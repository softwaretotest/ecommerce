...
export function renderDropdown_D(M_Class_Name_List, field_data) {
    ...
    const displayValue =
        selected_D[fieldname] !== undefined
            ...
            : defaultValue;
    // console.log(
    //     "Dropdown_D -- OUTSIDE useEffect -- defaultValue = ",
    //     defaultValue,
    // );

    // useEffect(() => {
    //     console.log(
    //         "Dropdown_D -- IN useEffect -- defaultValue = ",
    //         defaultValue,
    //     );
    //     set_selected_D(fieldname, defaultValue);
    // }, [fieldname]);
    ...
    useEffect(() => {
        ...
        if (!d_params) {
            ...
            d_params = find_NEW_D_Params_in_M_MAP(displayValue);
        }

        setD_Params_State(
            <D_Params D_NAME={displayValue} d_params={d_params} />,
        );
        if (is_wrong_d_params_in_backend) {
            D_HEAL(fieldname, displayValue, M_value, d_params, M_value_Service);
        }
    }, [selected_D]);
    // useEffect(() => {
