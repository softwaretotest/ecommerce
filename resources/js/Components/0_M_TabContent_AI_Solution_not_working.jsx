...
export default function TabContent({ M_Class_Name }) {
    ...
    // useEffect(() => {
    //     use_M_Store.getState().setTotalFieldsToProcess(entries.length);
    // }, [entries.length]);

    const healingRegistry = useRef({
        pendingFields: new Map(),
        totalFields: Object.keys(M_value).length, // จำนวนฟิลด์ทั้งหมดที่เรากำลังวน Loop
    });
                    ...
                    >
                        ...
                        {M_Class_Name === "f" && Array.isArray(field_data) && (
                            <Field
                                ...
                                healingRegistry={healingRegistry}
                            />
                        )}
